...
StatefulShellRoute _agentCenterShell(
  ...
  pageBuilder: (context, state, navigationShell) => _fadeThroughPage(
    state,
    AgentCenterShell(shell: navigationShell),
    pageKey: '/settings/agents',
  ),
  branches: [
    StatefulShellBranch(
      routes: [
...
/// changes within a section (for example switching conversations) and for
/// every move inside Settings.
///
...
/// in-place swap. Pass [pageKey] where a page's own identity is not the
/// current location: a route with sub-routes sees the full location too, so
/// a parent and the child stacked on it would otherwise claim the same key,
/// and a section shell whose tabs are separate routes would look like a new
/// page on every tab change.
///
/// Fade-through rather than a slide: Settings sections are sibling routes
/// entered with `go`, not pages pushed onto a stack, so there is no
/// forward/back direction to slide along — a directional transition would
/// play the same way coming back as going in, which is exactly the effect
/// this avoids.
CustomTransitionPage<void> _fadeThroughPage(
  ...
  key: ValueKey(pageKey ?? state.uri.toString()),
  child: child,
